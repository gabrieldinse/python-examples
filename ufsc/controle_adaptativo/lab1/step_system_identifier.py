...
def identify_first_orders(output, input_amplitude, sampling_rate, method):
...
def sundaresan(output, input_amplitude, sampling_rate):
    ...
    k1 = np.where(output >= 0.353 * output_steady_state)[0][0]
    k2 = np.where(output >= 0.853 * output_steady_state)[0][0]
    t1 = k1 / sampling_rate
    t2 = k2 / sampling_rate

    gain = (output_steady_state - output[0]) / input_amplitude
...
def nishikawa(output, input_amplitude, sampling_rate):
    ...
    for k, k_prev in zip(range(1, len(output), 1), range(len(output) - 1)):
        area0 += (output_steady_state - (output[k] + output[k_prev]) / 2) / sampling_rate

    time_array = np.arange(
        1 / sampling_rate, len(output) / sampling_rate, 1 / sampling_rate)
    t0 = area0 / (output_steady_state - output[0])
    ...
    gain = (output_steady_state - output[0]) / input_amplitude
    time_constant = area1 / (0.368 * (output_steady_state - output[0]))
    delay = max(t0 - time_constant, 0.0)
    return gain, time_constant, delay


def smith(output, input_amplitude, sampling_rate):
    output_steady_state = np.average(output[int(len(output) * 0.95):])
    k1 = np.where(output >= 0.283 * output_steady_state)[0][0]
    k2 = np.where(output >= 0.632 * output_steady_state)[0][0]
    t1 = k1 / sampling_rate
    t2 = k2 / sampling_rate
