...
for rho in rhos:
    ...
    fig1 = plt.figure(figsize=(14.5 / 2.54, 10.5 / 2.54))
    ax1 = plt.subplot(111)
    ...
    plt.title("Evolução temporal dos coeficientes estimados\npara "
              + r"$\rho = " + str(rho) + r"$" + " (Estimação de $2^a$ Ordem)",
              fontsize=14)
    ...
    # Salvar figura
    plt.savefig(f"./figuras/1b_seg_coefs_rho_{rho}.svg")
    ...
    plt.title(f'Comparação das respostas para\n' + r"$\rho = " + str(rho) + r"$"
              + " (Estimação de $2^a$ Ordem)", fontsize=14)
    plt.xlabel('Tempo (s)')
    plt.ylabel('Valor')
    ...
    # Salvar figura
    plt.savefig(f"./figuras/1b_seg_degrau_rho_{rho}.svg")


### Modelo de Primeira Ordem ###
for rho in rhos:
    P_ant = rho * I2  # P inicial
    ...
    for n in np.arange(0, len(time)):
        ...
        P = np.matmul(I2 - np.matmul(h, ut), P_ant)
    ...
    # ----------------------------------------------------------------------------
    # Função de transferência do sistema estimado, Ge(z)
    numz = np.array([0, theta[0, -1]])
    denz = np.array([1, theta[1, -1]])
    Ge = ctl.tf(numz, denz, T)
    print(f"\n\nFunção de transferência estimada para rho = {rho}:")
    print(Ge)
    print(f"Ganho rp: {Ge.dcgain()}")
    ...
    plt.title(
        ...
        + str(rho) + r"$" + " (Estimação de $1^a$ Ordem)", fontsize=14)
    plt.xlabel('Amostra', fontsize=10)
    plt.ylabel('Valor', fontsize=10)
    plt.xlim(left=amostras[0], right=amostras[-1])
    plt.grid()
    ...
    # Salvar figura
    plt.savefig(f"./figuras/1b_prim_coefs_rho_{rho}.svg")
    ...
    plt.step(time, y2_step, where='post', color='red',
             label='Resposta da planta estimada')
    plt.step(time, sinal_degrau, where='post', linestyle="dashed",
             color="green",
             label="Entrada do sistema (degrau unitário)")
    plt.title(
        f'Comparação das respostas para\n' + r"$\rho = " + str(rho) + r"$"
        + " (Estimação de $1^a$ Ordem)", fontsize=14)
    ...
    # Salvar figura
    plt.savefig(f"./figuras/1b_prim_degrau_rho_{rho}.svg")
