...
for n in np.arange(0, time.size):
    y_[n] = b1 * (su[n - 1] + pert[n - 1]) + b2 * (su[n - 2] + pert[n - 2]) \
                - a1 * y_[n - 1] - a2 * y_[n - 2]
    buffer = np.transpose(
        np.matrix([su[n - 1], su[n - 2], -y_[n - 1], -y_[n - 2]]))

    # estimador MQR
    e[n] = y_[n] - buffer.T * parametros[:, n - 1]
...
for n in np.arange(0, time.size):
    ...
    y[n] = b1 * (su[n - 1] + pert[n - 1]) + b2 * (su[n - 2] + pert[n - 2]) \
           - a1 * y[n - 1] - a2 * y[n - 2]
    buffer = np.transpose(
    ...
    r0 = 1
    r1 = b2e * (
                -a1e * b1e * b2e - a1e * b2e ** 2 + a2e * b1e ** 2
                + a2e * b1e * b2e + am1 * b2e ** 2 - am2 * b1e * b2e + b2e ** 2) / (
                     -a1e * b1e ** 2 * b2e - a1e * b1e * b2e ** 2
                     + a2e * b1e ** 3 + a2e * b1e ** 2 * b2e
                     + b1e * b2e ** 2 + b2e ** 3)
    s0 = (
                 a1e ** 2 * b1e * b2e + a1e ** 2 * b2e ** 2
                 - a1e * a2e * b1e ** 2 - a1e * a2e * b1e * b2e
                 - a1e * am1 * b1e * b2e - a1e * am1 * b2e ** 2
                 - a1e * b1e * b2e - a1e * b2e ** 2 + a2e * am1 * b1e ** 2
                 + a2e * am1 * b1e * b2e + a2e * b1e ** 2 - a2e * b2e ** 2
                 + am1 * b2e ** 2 + am2 * b2e ** 2 + b2e ** 2) / (
                     -a1e * b1e ** 2 * b2e - a1e * b1e * b2e ** 2
                     + a2e * b1e ** 3 + a2e * b1e ** 2 * b2e
                     + b1e * b2e ** 2 + b2e ** 3)
    s1 = (
                     -a1e ** 2 * b1e * b2e - a1e ** 2 * b2e ** 2
                     + a1e * a2e * b1e ** 2 + 2 * a1e * a2e * b1e * b2e
                     + a1e * a2e * b2e ** 2 + a1e * am1 * b2e ** 2
                     - a1e * am2 * b1e * b2e + a1e * b2e ** 2
                     - a2e ** 2 * b1e ** 2 - a2e ** 2 * b1e * b2e
                     - a2e * am1 * b1e * b2e - a2e * am1 * b2e ** 2
                     + a2e * am2 * b1e ** 2 + a2e * am2 * b1e * b2e) / (
                     -a1e * b1e ** 2 * b2e - a1e * b1e * b2e ** 2
                     + a2e * b1e ** 3 + a2e * b1e ** 2 * b2e + b1e * b2e ** 2
                     + b2e ** 3)
    s2 = a2e * (
                -a1e * b1e * b2e - a1e * b2e ** 2 + a2e * b1e ** 2
                + a2e * b1e * b2e + am1 * b2e ** 2 - am2 * b1e * b2e
                + b2e ** 2) / (-a1e * b1e ** 2 * b2e - a1e * b1e * b2e ** 2
                               + a2e * b1e ** 3 + a2e * b1e ** 2 * b2e
                               + b1e * b2e ** 2 + b2e ** 3)


    # blocos S(z) e T(z)
    ys[n] = s0 * y[n] + s1 * y[n - 1] + s2 * y[n - 2]
    rt[n] = (bm1 / b1e) * sr[n] + (bm2 / b1e) * sr[n - 1] - (b2e / b1e) * \
            rt[n - 1]
    ...
    # bloco 1/R(z)
    su[n] = (1 / r0) * se[n] + ((r0 - r1) / r0) * su[n - 1] \
            + (r1 / r0) * su[n - 2]
